"use client";

import Image from "next/image";
import Link from "next/link";

import { ContactTrigger } from "./ContactDialog";

function GitHubMark() {
  return (
    <svg
      aria-hidden="true"
      className="unified-footer-link-icon"
      focusable="false"
      viewBox="0 0 24 24"
    >
      <path
        d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.09-.745.083-.73.083-.73 1.205.084 1.84 1.237 1.84 1.237 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23a11.5 11.5 0 0 1 3.003-.404c1.02.005 2.04.138 3.003.404 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"
        fill="currentColor"
      />
    </svg>
  );
}

function FooterArrow() {
  return (
    <span aria-hidden="true" className="unified-footer-arrow">
      <svg focusable="false" viewBox="0 0 16 16">
        <path className="unified-footer-arrow-stem" d="M4 12 11 5" />
        <path d="M7.25 5H11v3.75" />
      </svg>
    </span>
  );
}

export function SiteFooter() {
  return (
    <footer className="unified-footer" aria-label="Site information">
      <div className="unified-footer-inner">
        <div className="unified-footer-primary">
          <Link
            className="unified-footer-brand"
            href="/"
            aria-label="GENII Foundation, home"
          >
            <Image
              alt="GENII"
              height={238}
              src="/brand/genii-wordmark-black.png"
              width={1135}
            />
            <span>Foundation</span>
          </Link>

          <p className="unified-footer-statement">For futures worth inhabiting.</p>

          <nav className="unified-footer-links" aria-label="Footer navigation">
            <a
              className="unified-footer-github-link"
              href="https://github.com/genii-foundation"
            >
              <GitHubMark />
              <span>GitHub</span>
            </a>
            <a href="https://www.coherence-thesis.com/">The Coherence Thesis</a>
            <ContactTrigger className="unified-footer-contact">
              <span>Contact</span>
              <FooterArrow />
            </ContactTrigger>
          </nav>
        </div>

        <div className="unified-footer-provenance">
          <p>© 2026 by GENII Foundation.</p>
          <p>
            Custodians:{" "}
            <a
              href="https://www.instagram.com/allelseis"
              rel="author noopener noreferrer"
              target="_blank"
            >
              Robert James Ryan III
            </a>
            {" & "}
            <a
              href="https://aubreyfalconer.com"
              rel="author noopener noreferrer"
              target="_blank"
            >
              Aubrey Falconer
            </a>
            .
          </p>
        </div>
      </div>
    </footer>
  );
}
